...
export default function CompanyDocumentUploadFormPage() {
  ...
  useEffect(() => {
    ...
    const loadForm = async () => {
      ...
      try {
        const [companiesResponse, documentsResponse, data] = await Promise.all([
          ...
          isEdit && id ? getCompanyDocumentUploadById(id) : Promise.resolve(null),
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.company_id?.id) {
      formData.append("company_id", String(form.company_id.id));
    }
